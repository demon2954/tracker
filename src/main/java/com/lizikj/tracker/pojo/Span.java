...
public class Span {
	 ...
	 * value possible. For example, gettimeofday or syncing nanoTime against a tick
	 * of currentTimeMillis.
	 * 
	 * For compatibilty with instrumentation that precede this field, collectors or
	 * span stores can derive this via Annotation.timestamp. For example,
	 ...
	 * 
	 * For compatibilty with instrumentation that precede this field, collectors or
	 * span stores can derive this by subtracting Annotation.timestamp. For example,
	 * SERVER_SEND.timestamp - SERVER_RECV.timestamp.
	 * 
	...
	public boolean equals(Object o) {
		...
		if (o instanceof Span) {
			Span that = (Span) o;
			return (this.trace_id == that.trace_id) && (this.name.equals(that.name)) && (this.id == that.id) && equal(this.parent_id, that.parent_id) && equal(this.timestamp, that.timestamp)
					&& equal(this.duration, that.duration) && equal(this.annotations, that.annotations) && equal(this.binary_annotations, that.binary_annotations) && equal(this.debug, that.debug);
		}
		return false;
	}
	...
	public zipkin.Span toZipkin() {
		...
		for (BinaryAnnotation a : getBinary_annotations()) {
			result.addBinaryAnnotation(zipkin.BinaryAnnotation.builder().key(a.key).value(a.value).type(zipkin.BinaryAnnotation.Type.fromValue(a.type.getValue())).endpoint(from(a.host)).build());
		}
		return result.build();
	}
	...
	private static zipkin.Endpoint from(Endpoint host) {
		if (host == null)
			return null;
		if (host.port == null)
			return zipkin.Endpoint.create(host.service_name, host.ipv4);
